<template>
  <div class="app-container">
    ...
    <div class="operator-container">
      <el-button class="filter-item" type="danger" @click="transferVisible = true">转账</el-button>
    </div>

    <el-tabs v-model="listQuery.type" @tab-click="handleFilter">
      <el-tab-pane label="全部" />
      <el-tab-pane label="转入" name="1" />
      <el-tab-pane label="转出" name="2" />
    </el-tabs>

    <!-- 查询结果 -->
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      <el-table-column align="center" label="转账类型" prop="type">
        <template slot-scope="scope">
          <el-tag :type="success">{{ scope.row.type == 1 ? '转入' : '转出' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="转账金额" prop="money" />
      <el-table-column align="center" label="账户余额" prop="balance" />
      <el-table-column align="center" label="详情" prop="detail" />
      <el-table-column align="center" label="转账时间" prop="addTime" />
    </el-table>

    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />

    <el-dialog :visible.sync="transferVisible" title="转账">
      <el-form ref="transferForm" :rules="rules" :model="transferForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="运营商" prop="transferTo">
          <template>
            <el-select v-model="transferForm.transferTo">
              <el-option
...
<script>
import { accountHistoryList, accountList, transfer } from '@/api/account'
...
export default {
  ...
  methods: {
    getList() {
      ...
      accountHistoryList(this.listQuery)
        .then(response => {
